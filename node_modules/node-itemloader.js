...
appLoader.prototype.loadImage = function(file, external_url, name, callback) {
...
appLoader.prototype.loadVideo = function(file, external_url, name, callback) {
	ffprobe(file, function(err, data){
		...
		for(i=0; i<data.streams.length; i++){
			if(data.streams[i].codec_type == "video"){
				...
				console.log("name: " + data.streams[i].width + "x" + data.streams[i].height);
				var appInstance = {
					id: null,
					title: name,
...
appLoader.prototype.manageAndLoadUploadedFile = function(file, callback) {
	...
	fs.rename(file.path, localPath, function(err) {
		...
		if(app === "image_viewer"){
			_this.loadImage(localPath, external_url, file.originalFilename, function(appInstance){
				callback(appInstance);
			});
		}
		else if(app === "movie_player"){
			_this.loadVideo(localPath, external_url, file.originalFilename, function(appInstance){
				callback(appInstance);
			});
		}
	});
