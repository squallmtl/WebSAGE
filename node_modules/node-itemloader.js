...
module.exports.loadApp = function(zip_path, zip_url, id, callback) {
	...
	fs.readFile(instuctionsFile, 'utf8', function(err, json_str) {
		...
		callback(new item(instructions.type, zipName, id, path.join(zip_url, instructions.main_script), null, 0, 0, instructions.width, instructions.height, aspect, now, zip_url+path.sep, objName), instructions);
	});
}

module.exports.loadZipApp = function(zip_path, zip_url, id, callback) {
...
module.exports.loadRemoteScreen = function(img_data, id, title, callback) {
	console.log(img_data.substring(0, 50));
	
	var buf = new Buffer(img_data);
	var b64 = buf.toString('base64');
	var data = new Buffer(b64, 'base64');
	...
	var aspect = info.width / info.height;
	var now = new Date();
	callback(new item("remote_screen", title, id, data, null, 0, 0, info.width, info.height, aspect, now, null, null));
}
